%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script: Extract subject-specific activated voxels within watershed fROIs
%
...
%
% Description:
% This script generates subject-specific “activated voxel” masks within a
% predefined set of watershed-derived fROIs (parcels). For each ROI and each
% subject, it intersects the subject’s first-level T-map with the ROI mask
% and then thresholds the ROI-restricted T-values either by:
%   (1) a fixed statistical threshold (p-value via corresponding T-values), or
%   (2) selecting the top X% of voxels (highest T-values) within the ROI.
%
% The outputs are binary NIfTI masks per subject/ROI (and threshold setting),
% suitable for downstream fROI time series extraction or ROI-based analyses.
% The script also records per-subject/ROI thresholding outcomes in a log
% table (e.g., number of suprathreshold voxels).
%
% Workflow:
% 1) Load ROI masks from a watershed ROI directory (one NIfTI per ROI).
% 2) For each ROI:
%    - Compute ROI size (# voxels).
% 3) For each contrast (e.g., spmT_0017 = 123vs0back):
%    - For each subject:
%      a) Load the subject’s first-level T-map.
%      b) Create an ROI-restricted T-map (Tmap .* ROI) and save it.
%      c) Threshold activated voxels within the ROI using one of:
%         - p_value mode:
%             * create whole-brain suprathreshold map (T > T_threshold)
%             * create ROI suprathreshold map: (T > T_threshold) .* ROI
%             * compute and log # suprathreshold voxels per threshold
%         - top_percent mode:
%             * rank ROI voxels by T-value (descending)
%             * select top X% voxels, save as binary ROI mask
%             * optionally repeat using only voxels with T > 0
%             * log # selected voxels and the max/min T in the selection
% 4) Save the log matrix as an Excel sheet for QC/summary reporting.
%
% Inputs:
% - Subject first-level T-maps:
%     <path>/sub-*/combine_ses/spmT_*.nii
% - Watershed ROI masks (one per ROI):
%     ROI_folder/*.nii
%
% Outputs:
% - For each contrast and ROI:
%     output_dir/<contrast>/<ROI_name>/<subject>.nii               (Tmap in ROI)
%     output_dir/<contrast>/<ROI_name>/<p>/<subject>.nii          (p-thresholded ROI voxels)
%     output_dir/<contrast>/<ROI_name>/topXpct/<subject>.nii       (top X% ROI voxels)
%     output_dir/<contrast>/<ROI_name>/topXpct_above0/<subject>.nii (top X% with T>0)
% - Log table:
%     output_dir/<contrast>/log_matrix.xlsx
%
% Key parameters:
% - threshold_type: 'p_value' or 'top_percent'
% - T_thresholds / p_thresholds: mapping from desired p-values to T cutoffs
% - top_percent: percentage of ROI voxels selected when using 'top_percent'
%
% Notes:
% - All subject T-maps and ROI masks must be in the same space/resolution.
% - In top_percent mode, voxels outside the ROI are set to -Inf to ensure
%   selection is restricted to ROI voxels only.
% - Update `path`, `output_dir`, and `ROI_folder` to match your environment.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
output_dir = '/results/fROI_Analysis/fROI_p_00001_s4to6fmap_brain_mask/mask_CER_PAR_IFG_ROI_s4fmap_brain_first_level_concatenated/selected_individual_activations';

% which contrast(s) to use to define "activated voxels" within each ROI
cons = {'spmT_0017'}; %17 for 123_vs_0back
con_names = {'123vs0back'};

% set ROI folder
ROI_folder = '/results/fROI_Analysis/fROI_p_00001_s4to6fmap_brain_mask/mask_CER_PAR_IFG_ROI_s4fmap_brain_first_level_concatenated/selected_Watershed_min2subj_123_vs_0back_0.0001_smooth6mm';
