%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script: Group-level DCM inference with PEB/BMR/BMA (true concatenated model)
%
...
%
% Description:
% This script performs group-level Bayesian analysis of estimated subject DCMs
% using SPM12’s Parametric Empirical Bayes (PEB) framework. Starting from a
% previously estimated GCM (Group Connectivity Model; one DCM per subject), it:
%
% 1) Builds a PEB model with an intercept-only design matrix (group mean).
% 2) Estimates the PEB on selected parameter fields (A, B, and C matrices).
% 3) Runs Bayesian Model Reduction (BMR) and Bayesian Model Averaging (BMA)
%    to identify the most likely reduced models and obtain robust group-level
%    parameter estimates.
% 4) Produces QC/diagnostic outputs (e.g., posterior model probabilities and
%    variance explained).
% 5) Exports a results table containing parameter names, human-readable
%    connection labels, posterior estimates (Ep), uncertainty (Cp), posterior
%    probabilities (Pp), and derived “Hz change” estimates for modulatory
%    parameters.
% 6) Computes Bayesian contrasts between modulatory parameters (B-matrix) to
%    directly compare condition-specific modulation strengths on the same
%    connection.
%
% Inputs:
% - Estimated GCM file (from subject-level DCM inversion):
%     GCM_folder/GCM_<analysis_name>.mat
%
% Outputs:
% - Intercept-only design matrix file:
%     GCM_folder/design_matrix_controls.mat
% - PEB results (A, B, C):
%     GCM_folder/PEB_A_B_C_<analysis_name>.mat
% - BMA/BMR results:
%     GCM_folder/BMA_A_B_C_<analysis_name>.mat
% - Model evidence plot (figure handle only unless saved manually)
% - Variance explained QC table:
%     GCM_folder/variance_explained_PEB_<analysis_name>.xlsx
% - Comprehensive parameter results table:
%     GCM_folder/results_table.xlsx
%
% Notes:
% - The design matrix here is intercept-only (group mean). To test covariates
%   (e.g., age, behavior, group), extend `design_matrix` accordingly.
% - `spm_dcm_peb_review` opens an interactive review GUI; run from a machine
%   with display support.
% - The “Hz change” computation approximates condition-dependent connectivity
%   changes by combining A and B parameters with representative input scaling
%   (based on the observed max/min of the DCM input regressors U).
% - This script assumes helper functions (e.g., pname_to_string) are available
%   on the MATLAB path.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
modulatory_conds = 2:4; % which conditions are modulatory inputs
driving_conds = 5;
