%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script: Group-level PEB/BMA for DCM (Session-modulation, concatenated data)
%
% Purpose:
% This script performs group-level inference on a set of subject-specific
% fMRI DCMs that have already been specified and estimated (stored in a GCM).
% It uses Parametric Empirical Bayes (PEB) to estimate group effects on the
% DCM parameters, then performs Bayesian Model Reduction (BMR) / Bayesian
% Model Averaging (BMA) to identify the most plausible reduced models and
% obtain posterior parameter estimates.
%
% Input:
% - GCM file: a cell array of estimated subject DCMs (GCM_*.mat)
%   located in GCM_folder.
% - Each DCM is assumed to contain:
%   * xY: ROI time series (first eigenvariates)
%   * U: experimental inputs (incl. task and/or session regressors)
%   * Estimated A/B/C parameters
%
% Key Steps:
% 1) Load an estimated GCM (one DCM per subject).
% 2) Define a (currently intercept-only) design matrix for PEB:
%    - design_matrix = ones(n_subjects,1) tests the group mean connectivity.
% 3) Run spm_dcm_peb on selected parameter fields:
%    - Here: {'A','B','C'} (intrinsic, modulatory, and driving parameters).
% 4) Run spm_dcm_peb_bmc to perform automatic search using BMR/BMA:
%    - Returns BMA (averaged parameters) and BMR (reduced model set + evidence).
% 5) Optional interactive review using spm_dcm_peb_review.
% 6) Compute and export diagnostics:
%    - Variance explained per subject (via spm_dcm_fmri_check).
% 7) Extract and tabulate BMA results into a readable table:
%    - Parameter names (Pnames), human-readable connection labels,
%      posterior means (Ep), posterior variances (diag(Cp)), posterior
%      probabilities (Pp).
% 8) (Optional) Compute "Hz change" for modulatory parameters:
%    - Approximates effective connectivity change induced by an input,
%      combining A and relevant B terms and the observed range of U.
% 9) (Optional) Bayesian contrasts between parameters:
%    - Pairwise posterior probability comparisons between modulators
%      (here: session_1 vs session_2) and between driving inputs.
%
% Outputs (saved in GCM_folder):
% - design_matrix_controls.mat
% - PEB_A_B_C_<analysis_name>.mat   (PEB structure + RCM)
% - BMA_A_B_C_<analysis_name>.mat   (BMA + BMR)
% - variance_explained_PEB_<analysis_name>.xlsx
% - results_table.xlsx             (parameter-level summary table)
%
% Notes / Assumptions:
% - The design_matrix is currently intercept-only; to test group differences
%   (e.g., sham vs active, covariates, behavioral scores), replace M.X with
%   an appropriate matrix and update X_labels accordingly.
% - 'modulatory_conds' and 'driving_conds' must match the ordering of inputs
%   (GCM{1}.U.name) used when specifying the DCMs.
% - pname_to_string() must be available on the MATLAB path. It converts SPM
%   Pnames (e.g., 'B(3,1,5)') into readable labels using ROI and input names.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
X_labels = {'Mean'};
cd(GCM_folder)
save('design_matrix_controls.mat','design_matrix','X_labels');
...
load([GCM_folder '/BMA_A_B_C_' analysis_name '.mat'])
...
%% Get percent variance explained (should get same values as in the last step)
...
writetable(varExp_table, [GCM_folder '/variance_explained_PEB_' analysis_name '.xlsx'])
...
modulatory_conds = 5:6; % which conditions are modulatory inputs
driving_conds = 2:4;
