%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script: Build concatenated nuisance regressor matrices across sessions
%         (fMRIPrep confounds + motion/FD QC + task structure + d' outliers)
%
...
%
% Description:
% This script creates subject-level nuisance regressor matrices for SPM by
% combining (concatenating) two n-back sessions (nback1 and nback2) into a
% single continuous regressor matrix (R). The regressors are derived from
% fMRIPrep confounds (motion parameters, FD, aCompCor), task block timing
% information (block order regressor), and optional behavioral outlier blocks
% (identified via d′ ≤ 0). It also generates QA plots for motion parameters
% and framewise displacement (FD), and writes a summary table of FD outliers.
%
% Key steps (per subject):
% 1) For each session (nback1, nback2):
%    - Load fMRIPrep confounds_timeseries.tsv.
%    - Build motion regressors (6 or 24; default 24 with derivatives/powers).
%    - Plot translation/rotation parameters for QA.
%    - Compute FD and create censoring (spike) regressors for volumes with
%      FD > FD_threshold.
%    - Plot FD for QA and store summary FD statistics (count/max/mean/SD).
%    - Optionally add the top N aCompCor components.
%    - Build a “block order” regressor using SPM timing CSVs (block starts
%      and durations), mapping each block to its volume range.
%    - If present, add d′-based block outlier regressors (one-hot vectors
%      marking volumes belonging to low-performance blocks).
%
% 2) Concatenate sessions into one continuous matrix:
%    - Motion parameters, aCompCor, and block-order regressors are stacked
%      vertically (session1; session2).
%    - Session-specific spike regressors (FD outliers) are concatenated with
%      zero-padding so that each session’s outlier columns affect only their
%      own time segment.
%    - Session-specific behavioral block-outlier regressors (if present) are
%      similarly concatenated with zero-padding.
%    - A session regressor is intentionally NOT added (because the design is
%      explicitly concatenated into one run).
%
% Inputs:
% - fMRIPrep confounds files:
%     <fmriprep_dir>/sub-*/ses-nback*/func/*confounds_timeseries.tsv
% - Timing CSVs:
%     onset_dir (block_starts) and dur_dir (block_durations)
% - Behavioral outlier blocks table:
%     outliar_blocks.csv  (code, session, order, nback)
%
% Outputs:
% - Concatenated nuisance regressor file per subject:
%     outdir/<sub>_ses-task-all.mat   (variable: R, volumes×regressors)
% - QA figures:
%     outdir_motionFigures/motion_<sub>_ses-*.png
%     outdir_motionFigures/FD_<sub>_ses-*.png
% - FD summary table (updated/appended):
%     outdir_motionFigures/FD_outliers.csv
%
% Notes:
% - Assumes 1420 volumes per session (used to build block order regressor and
%   zero-padding); concatenated run length becomes 2 × 1420.
% - All masks/timing files must correspond to the same subject/session naming.
...
outdir = '/results/fMRI_analysis/nuisance_regressors/motionRegs24_FD09_aCompCor10_order_withoutsess_concat';
if isempty(dir(outdir))
    mkdir(outdir)
end

outdir_motionFigures = '/results/fMRI_analysis/QA_motion_concat';
...
for iSub = 1:numel(sub_dirs)
    ...
    %%
    if sub_dirs(iSub).isdir == 1  && ~all(sub_code=='4004') % fix second partany(cellfun(@(x) all(join(["sub-ham",sub_code "_anat.html"], "",2)==x | join([sub_dirs(iSub).name ".html"], "",2)==x), {sub_dirs.name}))
        ...
        for iTask = 1:numel(tasks)
            ...
            %% session regressor -> do not add for concatenation
           session_reg = ones(1420,1)*(iTask-mean(1:length(tasks)));
          
            %% put stuff together
            R{iTask} = {motion_regs motion_outliers aCompCor_regs orders}
            ...
            if any(outblocks.code == str2double(part_code{:})) && any(outblocks.session == iTask)
                ...
                R{iTask} = [R{iTask} outliars_reg];
            end
        end

        %% Concatenation 
        % R1 is motion and same for everyone
        cR = [R{1,1}{1, 1};R{1,2}{1, 1}];

        % R2 is motion outliars and diff for each session
        if ~isempty(R{1,1}{1, 2})
            motout = [R{1,1}{1, 2};zeros(size(R{1,1}{1, 2}))];
            cR = [cR, motout];
        end

        if ~isempty(R{1,2}{1, 2})
            motout = [zeros(size(R{1,2}{1, 2})); R{1,2}{1, 2}];
            cR = [cR, motout];
        end

        % R3 is aCompCor and is same for everyone
        aCC = [R{1,1}{1, 3};R{1,2}{1, 3}];
        cR = [cR, aCC];

        % R4 is orders and same for everyone
        ord = [R{1,1}{1, 4};R{1,2}{1, 4}];
        cR = [cR, ord];

        % R5 is block outliars of dprime and it doesnt always exist
        if length(R{1,1})==5
            blkout = [R{1,1}{1, 5};zeros(size(R{1,1}{1, 5}))];
            cR = [cR, blkout];
        end

        if length(R{1,2})==5
            blkout = [zeros(size(R{1,2}{1, 5}));R{1,2}{1, 5}];
            cR = [cR, blkout];
        end
        ...
        %% save
        save([outdir '/' curr_sub '_ses-' 'task-all.mat'], 'R')
